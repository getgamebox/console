...
#pragma warning restore S4143
        ...
        public static string StripHtml(string str)
        {
            ...
            var htmlRegex = "[a-z-][^<>]*";
            var ret = new StringBuilder();
            var matches = Regex.Matches(str, $"<(({htmlRegex}) | /({htmlRegex})?)>",
            ...
            foreach (Match match in matches)
            {
                ...
                ret.Append(str.Substring(offset, position - offset));
                offset = position + text.Length;
            }

            ret.Append(str.Substring(offset));
        ...
        {
            // todo:
            return str.Aggregate(0, (r, c) => r + Width(c));
        }
        ...
        /// <param name="separator">The line is broken using the optional <paramref name="separator"/> parameter. default value is : <see cref="Environment.NewLine"/>.</param>
        /// <returns>Wrapped string.</returns>
        public static string WordWrap(string str, int width = 75, string separator = null)
        {
            if (string.IsNullOrEmpty(str) || width <= 0 || Width(str) <= width)
            {
                return str;
            }
            ...
            var prefaceBuilder = new StringBuilder();
            var prefaceWidth = 0;
            var backtrackingPosition = 0;
            var lastTruncatedPosition = 0;

            for (var position = 0; position < str.Length;)
            {
                var character = str[position];
                var characterWidth = Width(character);

                if (prefaceWidth + characterWidth <= width)
                {
                    if (characterWidth > 1 || character == ' ')
                    {
                        backtrackingPosition = position;
                    }

                    prefaceBuilder.Append(character);
                    prefaceWidth += characterWidth;
                    position++;
                    continue;
                }

                // 10% minimum cutoff limit
                if (backtrackingPosition > (lastTruncatedPosition + (int)(width * 0.1f)))
                {
                    var appendLength = backtrackingPosition - lastTruncatedPosition + 1;
                    var appendLastCharacterIsSpace = prefaceBuilder[appendLength - 1] == ' ';
                    var spaceOffset = 0;

                    // Truncated next character as the starting character, so we +1
                    lastTruncatedPosition = backtrackingPosition + 1;

                    // Use the trailing spaces as separators first.
                    if (appendLastCharacterIsSpace
                        && (lastTruncatedPosition < str.Length && str[lastTruncatedPosition] == ' '))
                    {
                        spaceOffset = 0;

                        // If the next character is a space we think it is a valid
                        // truncation position, so skip it.
                        lastTruncatedPosition += 1;
                    }
                    else if (appendLastCharacterIsSpace)
                    {
                        spaceOffset = -1;
                    }

                    result.Append(prefaceBuilder.ToString(0, appendLength + spaceOffset));
                    result.Append(separator);
                    prefaceBuilder.Clear();
                    prefaceWidth = 0;

                    position = lastTruncatedPosition;
                    continue;
                }

                result.Append(prefaceBuilder.ToString());
                result.Append(separator);
                lastTruncatedPosition += prefaceBuilder.Length;
                backtrackingPosition = lastTruncatedPosition;
                position = lastTruncatedPosition;
